...
namespace Rundo.RuntimeEditor.Data.UiDataMapper
{
    public class UiDataMapper
    {
        ...
        public UiDataMapperElementInstance Create(Type valueType, string label)
        {
            if (UiElementsContent == null)
                throw new Exception($"Need to call SetUiElementsContent() before Create");
            
            var instance = UiElementsFactory.GetUiDataMapperElement(valueType, label);
        ...
        public UiDataMapperElementInstance<TValue> Create<TValue>(string label)
        {
            return Create(typeof(TValue), label) as UiDataMapperElementInstance<TValue>;
        }
